        ...
        var sandbox = {
            ...
            getState:       function (id, callback) {
                if (typeof callback === 'function') {
                    ...
                    if (adapter.config.subscribe) {
                        sandbox.log('Cannot use sync getState, use callback instead getState("' + id + '", function (err, state){}); or disable the "' + words._('Do not subscribe all states on start:') + '" option in instance configuration.', 'error');
                    } else {
                        if (states[id]) {
