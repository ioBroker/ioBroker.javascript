 ...
 * @param {boolean} isForGlobalDeclarations Whether this transformation is used to generate declarations for a global script
 * @returns {string}
 */
function transformScriptBeforeCompilation(source, isGlobal, isForGlobalDeclarations = isGlobal) {
    /**
        ...
        return (sourceFile) =>
            ts.visitNode(sourceFile, (node) => {
                if (ts.isSourceFile(node)) {
                    ...
                    // When generating declarations for global scripts, we need to do some tricks
                    if (isGlobal && isForGlobalDeclarations) {
                        ...
                        // 2. We need to transform the generated declarations to use `declare global`
                    }
                    const hasImport = hoistedStatements.some(s => canBeExported(s))
