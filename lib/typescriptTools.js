...
function resolveTypings(pkg, adapterScopedPackageName, wrapInDeclareModule) {
    ...
    const normalizeImportPath = filename =>
        path
            .normalize(
    ...
    const matchAllImports = string =>
        [...matchAll(importDtsRegex, string), ...matchAll(pathReferenceRegex, string)].map(groups => groups[0]);
...
function mustBeHoisted(s, isGlobal) {
    return (
        ...
        (isGlobal &&
            ...
            (ts.isClassDeclaration(s) || ts.isFunctionDeclaration(s))) ||
        // and declare ... / export ... statements
        (s.modifiers &&
            s.modifiers.some(s => s.kind === ts.SyntaxKind.DeclareKeyword || s.kind === ts.SyntaxKind.ExportKeyword))
    );
}
...
function transformScriptBeforeCompilation(source, isGlobal) {
            ...
            ts.visitNode(sourceFile, node => {
                if (ts.isSourceFile(node)) {
                    ...
                    const hasTLA = node.statements.some(
                        s => ts.isExpressionStatement(s) && s.expression.kind === ts.SyntaxKind.AwaitExpression,
                    );
                    // Move all statements to the top of the file that cannot appear in a function body
                    let hoistedStatements = hasTLA
                        ? ts.factory.createNodeArray(nonAugmentations.filter(s => mustBeHoisted(s, isGlobal)))
                        : ts.factory.createNodeArray(nonAugmentations);
                    ...
                    const needsEmptyExport =
                        ...
                        (!(
                            ...
                            hoistedStatements.some(
                                s => s.modifiers && s.modifiers.some(m => m.kind === ts.SyntaxKind.ExportKeyword),
                            )
                        ) ||
    ...
    const sourceFile = ts.createSourceFile('index.ts', source, ts.ScriptTarget.ESNext, /* setParentNodes */ true);
...
function transformGlobalDeclarations(decl) {
    ...
    const sourceFile = ts.createSourceFile('index.d.ts', decl, ts.ScriptTarget.ESNext, /* setParentNodes */ true);
