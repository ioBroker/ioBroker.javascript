...
function stringOrRegExpCompare(name, pattern) {
    ...
    if (field instanceof RegExp || field.source) {
...
var patternCompareFunctions = {
    ...
    id: function (pattern) {
        var pid = pattern.id;
        if (pid instanceof RegExp || pid.source) {
            return function (event) {
    ...
    enumName: function (pattern) {
        ...
        if (penumName instanceof RegExp) {
            return function (event) {
                for (var j = 0; j < event.enumNames.length; j++) {
                    //if (event.enumNames[j].match (penumName)) {
