...
function sandBox(script, name, verbose, debug, context) {
    ...
    function setStateHelper(sandbox, isBinary, id, state, isAck, callback) {
        ...
        if (adapter.config.subscribe && (!isObject(state) || state.val === undefined)) {
            state = {val: state};
                ...
                if (objects[id].type === 'state') {
                    ...
                    if (debug) {
                        ...
                        setStateFunc(id, state, err => {
                            ...
                            // If adapter holds all states in memory
                            if (!err && adapter.config.subscribe) {
                                states[id] = state; // store actual state to make possible to process value in callback
                            }
                            if (typeof callback === 'function') {
                                try {
                                    callback.call(sandbox);
    ...
    const sandbox = {
        ...
        messageTo:      function (target, data, options, callback) {
            ...
            if (target.instance || target.instance === 0) {
                ...
                context.adapter.objects.getObjectView('system', 'instance', {startkey: 'system.adapter.javascript.', endkey: 'system.adapter.javascript.\u9999'}, options, (err, res) => {
