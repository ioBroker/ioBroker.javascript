...
function sandBox(script, name, verbose, debug, context) {
    ...
    const sandbox = {
        ...
        messageTo:      function (target, data, options, callback) {
            ...
            if (target.instance) {
                if (target.instance.startsWith('system.adapter.')) {
                    target.instance = target.instance.substring('system.adapter.'.length);
                }

                adapter.sendTo(target.instance, 'jsMessageBus', {message: target.message, data}, timeout && function (result) {
                ...
                this.objects.getObjectView('system', 'instance', {startkey: 'system.adapter.javascript.', endkey: 'system.adapter.javascript.\u9999'}, options, (err, res) => {
                    const len = 'system.adapter.'.length;
                    const instances = res.rows.map(item => item.id.substring(len));

                    instances.forEach(instance => {
                        adapter.sendTo(instance, 'jsMessageBus', {message: target.message, data}, timeout && function (result) {
                            timeout && clearTimeout(timeout);
        ...
        onMessageUnregister: function (id) {
