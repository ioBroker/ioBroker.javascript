...
function sandBox(script, name, verbose, debug, context) {
    ...
    const sandbox = {
        ...
        schedule:       function (pattern, callback) {
            ...
            if ((typeof pattern === 'string' && pattern[0] === '{') || (typeof pattern === 'object' && pattern.period)) {
                if (sandbox.verbose) sandbox.log('schedule(wizard=' + (typeof pattern === 'object' ? JSON.stringify(pattern) : pattern) + ')', 'info');
        ...
        clearSchedule:  function (schedule) {
            if (context.scheduler.get(schedule)) {
                if (sandbox.verbose) sandbox.log('clearSchedule() => wizard cleared', 'info');
        ...
        _setState:      function (isBinary, id, state, isAck, callback) {
        ...
        setState:       function (id, state, isAck, callback) {
            return sandbox._setState(false, id, state, isAck, callback);
        },
        setBinaryState: function (id, state, callback) {
            return sandbox._setState(true, id, state, callback);
        },
