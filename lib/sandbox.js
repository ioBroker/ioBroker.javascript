...
function sandBox(script, name, verbose, debug, context) {
    ...
    const sandbox = {
        ...
        onLog: function (severity, callback) {
            ...
            sandbox.verbose && sandbox.log(`onLog(severity=${severity}, id=${handler.id}) - logSubscriptions=${sandbox.__engine.__subscriptionsLog}`, 'info');
        ...
        onLogUnregister: function (idOrCallbackOrSeverity) {
            ...
            if (context.logSubscriptions?.[sandbox.scriptName]) {
                ...
                sandbox.verbose && sandbox.log(`onLogUnregister(idOrCallbackOrSeverity=${idOrCallbackOrSeverity}) - logSubscriptions=${sandbox.__engine.__subscriptionsLog}`, 'info');
                ...
                for (let i = 0; i < context.logSubscriptions[sandbox.scriptName].length ; i++) {
                    if (context.logSubscriptions[sandbox.scriptName][i].cb       === idOrCallbackOrSeverity ||
                        ...
                        sandbox.verbose && sandbox.log(`onLogUnregister(idOrCallbackOrSeverity=${idOrCallbackOrSeverity}, removing id=${context.logSubscriptions[sandbox.scriptName][i].id})`, 'info');
                        ...
                        sandbox.verbose && sandbox.log(`onLogUnregister(idOrCallbackOrSeverity=${idOrCallbackOrSeverity}) NOT = ${JSON.stringify(context.logSubscriptions[sandbox.scriptName][i])}`, 'info');
                    }
                }
            }
        ...
        createTempFile: function (fileName, data) {
            ...
            sandbox.verbose && sandbox.log(`createTempFile(fileName=${fileName}, fileDir=${fileDir}, filePath=${filePath})`, 'info');
                ...
                const init = () => {
                    ...
                    for (const objId of members) {
                        if (!Object.keys(subscriptions).includes(objId)) {
                            if (objects?.[objId]?.type === 'state') { // Just subscribe to states
                                subscriptions[objId] = sandbox.subscribe(objId, callback); // TODO: more features
                            }
                        }
                    }

                    sandbox.verbose && sandbox.log(`onEnumMembers(id=${id}, members=${JSON.stringify(Object.keys(subscriptions))})`, 'info');
                };
        ...
        onFile: function (id, fileNamePattern, withFile, callback) {
            ...
            const subs = {
                ...
                callback: (id, fileName, size, withFile) => {
                    try {
                        sandbox.verbose && sandbox.log(`onFile changed(id=${id}, fileName=${fileName}, size=${size})`, 'info');
        ...
        setStateDelayed: function (id, state, isAck, delay, clearRunning, callback) {
            ...
            sandbox.verbose && sandbox.log(`setStateDelayed(id=${id}, state=${JSON.stringify(state)}, isAck=${isAck}, delay=${delay}, clearRunning=${clearRunning})`, 'info');
                ...
                context.adapter.getObjectView('system', 'instance', {startkey: 'system.adapter.javascript.', endkey: 'system.adapter.javascript.\u9999'}, options, (err, res) => {
                    ...
                    instances.forEach(instance => {
                        adapter.sendTo(instance, 'jsMessageBus', { message: target.message, script: target.script, data }, timeout && function (result) {
                            ...
                            if (typeof callback === 'function') {
                                sandbox.verbose && result?.result && sandbox.log(`messageTo result => ${JSON.stringify(result)}`, 'info');
