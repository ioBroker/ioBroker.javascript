...
function sandBox(script, name, verbose, debug, context) {
    ...
    let sandbox = {
        ...
        $:         function (selector) {
            ...
            let pass;
            if (name === 'channel') {
                for (let id in context.channels) {
                ...
                let r = (name && name !== 'state') ? new RegExp('^' + name.replace(/\./g, '\\.').replace(/\*/g, '.*') + '$') : null;

                // state
                for (let s = 0; s < context.stateIds.length; s++) {
                    let id = context.stateIds[s];
        ...
        exec:      function (cmd, callback) {
            if (!adapter.config.enableExec) {
                let error = 'exec is not available. Please enable "Enable Exec" option in instance settings';
        ...
        subscribe: function (pattern, callbackOrId, value) {
            ...
            if (pattern && pattern.id && Array.isArray(pattern.id)) {
                ...
                for (let tt = 0; tt < pattern.id.length; tt++) {
                    let pa = JSON.parse(JSON.stringify(pattern));
                    pa.id = pattern.id[tt];
            ...
            if (pattern.id && typeof pattern.id === 'string') {
                let parts = pattern.id.split('.');
                let a = parts[0] + '.' + parts[1];
                let _adapter = 'system.adapter.' + a;

                if (objects[_adapter] && objects[_adapter].common && objects[_adapter].common.subscribable) {
                    let alive = 'system.adapter.' + a + '.alive';
                    context.adapterSubs[alive] = context.adapterSubs[alive] || [];

                    let subExists = context.adapterSubs[alive].filter(function(sub){
        ...
        schedule:       function (pattern, callback) {
            ...
            if (pattern.astro) {
                ...
                if (typeof pattern === 'string') {
                    let parts = pattern.replace(/\s+/g, ' ').split(' ');
                    if (parts.length >= 5 && parts[5] >= 7) parts[5] = 0;
                    pattern = parts.join(' ');
                }
                let schedule = nodeSchedule.scheduleJob(pattern, function () {
                    try {
                        callback.call(sandbox);
                    } catch (e) {
                        errorInCallback(e); //adapter.log.error('Error in callback: ' + e)
        ...
        isAstroDay:     function () {
            ...
            let dayBegin = sandbox.getAstroDate('sunrise');
            let dayEnd   = sandbox.getAstroDate('sunset');
        ...
        setState:       function (id, state, isAck, callback) {
            ...
            let common = objects[id] ? objects[id].common : null;
            if (common &&
                common.type &&
                common.type !== 'mixed' &&
                common.type !== 'file'  &&
        ...
        setStateDelayed: function (id, state, isAck, delay, clearRunning, callback) {
            ...
            if (!delay) {
                ...
                // Start timeout
                let timer = setTimeout(function (_timerId, _id, _state, _isAck) {
        ...
        getObject:      function (id, enumName) {
            if (!objects[id]) {
                ...
                let e = eventObj.getObjectEnumsSync(id);
                ...
                if (typeof enumName === 'string') {
                    let r = new RegExp('^enum\.' + enumName + '\.');
