            ...
            if (enums.includes(id)) {
                ...
                const init = () => {
                    ...
                    for (let i = 0; i < subscriptions.length; i++) {
                        sandbox.unsubscribe(subscriptions[i]);
                    ...
                    for (const member of members) {
                        subscriptions.push(sandbox.subscribe(member, callback));
                    }

                    sandbox.verbose && sandbox.log(`onEnumMembers(id=${id}, members=${JSON.stringify(common?.members)}, subscriptions=${subscriptions.length})`, 'info');
                };
