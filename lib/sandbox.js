...
function sandBox(script, name, verbose, debug, context) {
    ...
    const sandbox = {
        ...
        log:            function (msg, severity) {
            ...
            if (debugMode) {
                if (severity === 'info' || severity === 'debug' || severity === 'silly') {
                    severity = 'log';
                    console[severity](`${name}: ${msg}`);
                }
            } else {
        ...
        getStateAsync:  function (id) {
            return new Promise((resolve, reject) => {
                if (id.includes('.')) {
                    adapter.getForeignState(id, (err, state) => err ? reject(err) : resolve(state));
                } else {
                    adapter.getState(id, (err, state) => err ? reject(err) : resolve(state));
                }
            });
        },
