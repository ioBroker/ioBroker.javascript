        ...
        onEnumMembers: function (id, callback) {
            if (enums.includes(id)) {
                ...
                const init = () => {
                    ...
                    for (const [obiId, subscription] of Object.entries(subscriptions)) {
                        if (!members.includes(obiId)) {
                            sandbox.unsubscribe(subscription);
                            delete subscriptions[obiId];
                        }
                    }

                    // Subscribe to all members of enum
                    for (const member of members) {
                        if (!Object.keys(subscriptions).includes(member)) {
                            subscriptions[member] = sandbox.subscribe(member, callback); // TODO: more features
                        }
                    }
