...
const words = require("./words"); // Get common adapter utils
...
function sandBox(script, name, verbose, debug, context) {
    ...
    function setStateHelper(sandbox, isBinary, id, state, isAck, callback) {
        let setStateFunc;
        if (isBinary) {
            sandbox.log(`setBinaryState(id=${id}) is deprecated. Please use Files instead!`, 'info');
    ...
    const sandbox = {
        ...
        $: function (selector) {
            ...
            result.getBinaryState = function (callback) {
                sandbox.log(`$.getBinaryState(id=${this[0]}) is deprecated. Please use Files instead!`, 'info');
                if (adapter.config.subscribe) {
                    if (typeof callback !== 'function') {
                        sandbox.log('You cannot use this function synchronous', 'error');
                    } else {
            ...
            result.getBinaryStateAsync = function() {
                sandbox.log(`$.getBinaryStateAsync(id=${this[0]}) is deprecated. Please use Files instead!`, 'info');
                if (adapter.config.subscribe) {
        ...
        subscribe: function (pattern, callbackOrId, value) {
            ...
            if (pattern.id !== undefined && !pattern.id) {
                adapter.log.error(`${name} Error by subscription (trigger): empty ID defined. All states matched.`);
                return;
            } else if (typeof pattern.id === 'boolean' || typeof pattern.id === 'number') {
                adapter.log.error(`${name} Error by subscription (trigger): Wrong ID of type boolean or number.`);
                return;
            }
        ...
        getBinaryState: function (id, callback) {
            sandbox.log(`getBinaryState(id=${id}) is deprecated. Please use Files instead!`, 'info');
            if (typeof callback === 'function') {
                if (typeof adapter.getForeignBinaryState === 'function') {
        ...
        createState: function (name, initValue, forceCreation, common, native, callback) {
            ...
            adapter.getForeignObject(id,  async (err, obj) => {
                ...
                if (err || !obj || forceCreation) {
                    ...
                    adapter.setForeignObject(id, newObj, async (err) => {
                        if (err) {
        ...
        sendToAsync: function (_adapter, cmd, msg) {
            return new Promise((resolve, reject) => {
                sandbox.sendTo(_adapter, cmd, msg, res => {
                    if (!res || res.error) {
        ...
        getHistory: function (instance, options, callback) {
            ...
            adapter.sendTo(instance, 'getHistory', {id: options.id, options: options}, function (result) {
                ...
                if (sandbox.verbose && result && result.error)  sandbox.log(`getHistory => ${result.error}`, 'error');
                if (sandbox.verbose && result && result.result) sandbox.log(`getHistory => ${result.result.length} items`, 'debug');
