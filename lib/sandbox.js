...
function sandBox(script, name, verbose, debug, context) {
    ...
    function validIdForAutomaticFolderCreation(id) {
        return id.startsWith('javascript.') || id.startsWith('0_userdata.0.');
    }
    ...
    const sandbox = {
        ...
        subscribe: function (pattern, callbackOrId, value) {
            ...
            if (pattern.id !== undefined && !pattern.id) {
                adapter.log.error(`${name} Error by subscription: empty ID defined. All states matched.`);
                return;
            } else if (typeof pattern.id === 'boolean' || typeof pattern.id === 'number') {
                adapter.log.error(`${name} Error by subscription: ID of type boolean or number is not supported.`);
                return;
            }
        ...
        onFile: function (id, fileNamePattern, withFile, callback) {
            ...
            let fileRegEx;
            if (id.includes('*')) {
                idRegEx = new RegExp(tools.pattern2RegEx(id));
            }
            if (fileNamePattern.includes('*')) {
                fileRegEx = new RegExp(tools.pattern2RegEx(fileNamePattern));
            }
        ...
        onMessage: function (messageName, callback) {
            if (typeof callback !== 'function') {
                ...
                sandbox.__engine.__subscriptionsMessages += 1;
                            ...
                            for (let i = 0; i < ctx[messageName].length; i++) {
                                if (ctx[messageName][i].id === idOrName) {
                                    ...
                                    if (!ctx[messageName].length) {
                                        ...
                                        sandbox.__engine.__subscriptionsMessages--;
                                    }
                                    found = true;
                                    break;
                                }
