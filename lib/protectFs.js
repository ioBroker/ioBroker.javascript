...
const ProtectFs = function (log, ioBrokerDataDir) {
    function checkObjectsJson(file) {
        ...
        if (filePath.includes(`-data${path.sep}objects.json`) || filePath.includes(`-data${path.sep}objects.jsonl`)) {
            if (log) {
        ...
        } else if (filePath.startsWith(path.join(ioBrokerDataDir, 'files'))) {
            if (log) {
                log.error(`May not read ${file} - use writeFile/readFile instead`);
            } else {
                console.error(`May not read ${file} - use writeFile/readFile instead`);
            }
            throw new Error('Permission denied');
        }
    }
