...
const { astroList, astroListLow } = require('./consts');
class Scheduler {
    constructor(log, DateTest, suncalc, latitude, longitude) {
        ...
        this.astroListLow = astroListLow;
    }
    ...
    checkSchedule(context, schedule) {
        ...
        if (schedule.time) {
            ...
            if (start && (astroEvent = this._isAstro(start))) {
                times = this.suncalc.getTimes(now, this.latitude, this.longitude);
                start = times[astroEvent];
                start = start.getHours() * 60 + start.getMinutes();
            }

            let end = schedule.time.end;

            if (end && (astroEvent = this._isAstro(end))) {
                times = times || this.suncalc.getTimes(now, this.latitude, this.longitude);
                end = times[astroEvent];
    ...
    _isAstro(ev) {
        if (typeof ev === 'string') {
            const pos = this.astroListLow.indexOf(ev.toLowerCase());
            if (pos > -1) {
