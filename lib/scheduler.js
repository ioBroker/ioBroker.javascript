...
class Scheduler {
    ...
    add(schedule, scriptName, cb) {
        ...
        if (sch.valid) {
            ...
            if (typeof sch.period.days === 'number' && sch.period.days > 1) {
                ...
                sch.valid.fromDate = sch.valid.from
        ...
        if (sch.period.months && typeof sch.period.months !== 'number') {
    ...
    remove(id) {
        ...
        } else if (typeof id === 'string' && this.list[id]) {
            delete this.list[id];
            this.recalculate();
            return true;
        } else {
    ...
    getList() {
        return Object.keys(this.list).map(id => ({
            ...
            scriptId: this.list[id].scriptId,
        }));
    }
