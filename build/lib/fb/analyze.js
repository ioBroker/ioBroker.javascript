...
function isLoop(component, edges) {
    return component.length > 1 || !!edges.get(component[0])?.has(component[0]);
}
function buildEdges(ids, links, skip) {
...
function analyzeGraph(graph) {
    ...
    for (const link of graph.links) {
        ...
        const output = from && (0, graph_1.getOutputs)(from.block, from.def).find(pin => pin.id === link.from[1]);
        const input = to && (0, graph_1.getInputs)(to.block, to.def).find(pin => pin.id === link.to[1]);
        if (!output || !input) {
            issues.push({
                severity: 'error',
        ...
        if (!(0, graph_1.isCompatible)(output.type, input.type)) {
            issues.push({
                severity: 'error',
    ...
    const breakable = (link) => !!nodes.get(link.from[0])?.def.stateful &&
