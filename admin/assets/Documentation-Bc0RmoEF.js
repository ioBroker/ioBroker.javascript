...
`;n();var J={en:K,de:q},Y=`https://github.com/ioBroker/ioBroker.javascript/blob/master/docs/`,ie=`Editor.docScroll`,X=`Editor.docContents`,ae=1500,oe=`ui-monospace, "Cascadia Code", Consolas, monospace`,Z={};function se(e){let t=J[e]?e:`en`;if(!Z[t]){let e=L(I(J[t]));Z[t]={lang:t,blocks:e,entries:z(e)}}return Z[t]}function Q(e){try{return window.localStorage.getItem(e)}catch{return null}}function $(e,t){try{window.localStorage.setItem(e,t)}catch{}}function ce({onClose:e,word:n}){let{lang:r,blocks:p,entries:C}=se(g.getLanguage()),w=`${ie}.${r}`,T=b(null),E=b(null),D=b(0),O=b(null),[k,A]=o(()=>Q(X)!==`false`),[j,M]=o(``),[N,P]=o(``),F=S(()=>ne(C,j),[C,j]),I=()=>{let e=T.current;if(!e)return;let t=e.scrollTop+24,n=``;for(let r of Array.from(e.querySelectorAll(`[data-level]`)))if(!(Number(r.dataset.level)>3)){if(r.offsetTop>t)break;n=r.id.substring(V.length)}e.scrollTop+e.clientHeight>=e.scrollHeight-4&&O.current&&Date.now()-O.current.at<ae&&(n=O.current.anchor),P(n)},L=(e,t=!0)=>{var n;let r=(n=T.current)==null?void 0:n.querySelector(`#${CSS.escape(V+e)}`);r&&(O.current={anchor:e,at:Date.now()},P(e),r.scrollIntoView({block:`start`,behavior:t?`smooth`:`auto`}))};d(()=>{let e=B(C,n),t=Number(Q(w)),r=0,i=requestAnimationFrame(()=>{r=requestAnimationFrame(()=>{e?L(e,!1):T.current&&Number.isFinite(t)&&t>0&&(T.current.scrollTop=t),I()})});return()=>{cancelAnimationFrame(i),cancelAnimationFrame(r),cancelAnimationFrame(D.current)}},[]),d(()=>{if(N){var e;(e=E.current)==null||(e=e.querySelector(`[data-anchor="${CSS.escape(N)}"]`))==null||e.scrollIntoView({block:`nearest`})}},[N,k]);let R=()=>{D.current||=requestAnimationFrame(()=>{var e;D.current=0,$(w,String(((e=T.current)==null?void 0:e.scrollTop)||0)),I()})},z=S(()=>m(G,{blocks:p,base:`${Y}${r}/`,onAnchor:e=>L(e)}),[p,r]);return y(v,{open:!0,fullWidth:!0,maxWidth:`lg`,onClose:e,slotProps:{paper:{sx:{height:`calc(100% - 64px)`}}},children:[y(f,{sx:{display:`flex`,alignItems:`center`,gap:1,pb:1},children:[m(t,{title:g.t(`Contents`),slotProps:{popper:{sx:{pointerEvents:`none`}}},children:m(h,{size:`small`,color:k?`primary`:`default`,onClick:()=>{$(X,String(!k)),A(!k)},children:m(te,{})})}),g.t(`Documentation`)]}),y(_,{dividers:!0,sx:{display:`flex`,p:0,minHeight:0},children:[k?y(i,{sx:{flex:`0 0 280px`,minWidth:0,display:`flex`,flexDirection:`column`,minHeight:0,borderRight:1,borderColor:`divider`},children:[m(i,{sx:{p:1},children:m(a,{autoFocus:!0,fullWidth:!0,size:`small`,variant:`outlined`,placeholder:g.t(`Filter`),value:j,onChange:e=>M(e.target.value),onKeyDown:e=>{e.key===`Enter`&&F.length?L(F[0].anchor):e.key===`Escape`&&j&&(e.stopPropagation(),M(``))},slotProps:{input:{endAdornment:j?m(u,{position:`end`,children:m(h,{size:`small`,onClick:()=>M(``),children:m(s,{fontSize:`small`})})}):null}}})}),y(i,{ref:E,sx:{flex:1,overflowY:`auto`,pb:1},children:[F.map(e=>y(i,{"data-anchor":e.anchor,title:e.text,onClick:()=>L(e.anchor),sx:{display:`flex`,alignItems:`baseline`,gap:1,pl:e.level<3?1.5:3,pr:1,py:.4,mt:+(e.level<3&&!j),cursor:`pointer`,fontSize:e.level<3?14:13,fontWeight:e.level<3?500:400,color:e.anchor===N?`primary.main`:`text.primary`,bgcolor:e.anchor===N?`action.selected`:void 0,"&:hover":{bgcolor:`action.hover`}},children:[m(i,{component:`span`,sx:{flexShrink:+!e.code,fontFamily:e.code?oe:void 0,overflow:`hidden`,textOverflow:`ellipsis`,whiteSpace:e.code?`nowrap`:void 0,maxWidth:`100%`},children:e.name}),e.description?m(i,{component:`span`,sx:{minWidth:0,fontSize:12,color:`text.secondary`,overflow:`hidden`,textOverflow:`ellipsis`,whiteSpace:`nowrap`},children:e.description}):null]},e.anchor)),F.length?null:m(c,{sx:{px:1.5,py:1,fontSize:13,color:`text.secondary`},children:g.t(`Nothing found`)})]})]}):null,m(i,{ref:T,onScroll:R,sx:{flex:1,minWidth:0,overflowY:`auto`,position:`relative`,px:3,py:2},children:z})]}),m(ee,{children:m(l,{variant:`contained`,color:`grey`,startIcon:m(x,{}),onClick:e,children:g.t(`Close`)})})]})}export{ce as default};