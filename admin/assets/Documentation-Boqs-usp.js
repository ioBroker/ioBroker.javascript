...
`;i();var q={en:G,de:K},J=`https://github.com/ioBroker/ioBroker.javascript/blob/master/docs/`,ae=`Editor.docScroll`,Y=`Editor.docContents`,oe=1500,X=`ui-monospace, "Cascadia Code", Consolas, monospace`,Z={};function se(e){let t=q[e]?e:`en`;if(!Z[t]){let e=L(I(q[t]));Z[t]={lang:t,blocks:e,entries:z(e)}}return Z[t]}function Q(e){try{return window.localStorage.getItem(e)}catch{return null}}function $(e,t){try{window.localStorage.setItem(e,t)}catch{}}function ce({onClose:i,word:o}){let{lang:l,blocks:y,entries:C}=se(f.getLanguage()),w=`${ae}.${l}`,T=e(null),E=e(null),D=e(0),O=e(null),[k,A]=v(()=>Q(Y)!==`false`),[j,M]=v(``),[N,P]=v(``),F=b(()=>re(C,j),[C,j]),I=()=>{let e=T.current;if(!e)return;let t=e.scrollTop+24,n=``;for(let r of Array.from(e.querySelectorAll(`[data-level]`)))if(!(Number(r.dataset.level)>3)){if(r.offsetTop>t)break;n=r.id.substring(B.length)}e.scrollTop+e.clientHeight>=e.scrollHeight-4&&O.current&&Date.now()-O.current.at<oe&&(n=O.current.anchor),P(n)},L=(e,t=!0)=>{var n;let r=(n=T.current)==null?void 0:n.querySelector(`#${CSS.escape(B+e)}`);r&&(O.current={anchor:e,at:Date.now()},P(e),r.scrollIntoView({block:`start`,behavior:t?`smooth`:`auto`}))};x(()=>{let e=ne(C,o),t=Number(Q(w)),n=0,r=requestAnimationFrame(()=>{n=requestAnimationFrame(()=>{e?L(e,!1):T.current&&Number.isFinite(t)&&t>0&&(T.current.scrollTop=t),I()})});return()=>{cancelAnimationFrame(r),cancelAnimationFrame(n),cancelAnimationFrame(D.current)}},[]),x(()=>{if(N){var e;(e=E.current)==null||(e=e.querySelector(`[data-anchor="${CSS.escape(N)}"]`))==null||e.scrollIntoView({block:`nearest`})}},[N,k]);let R=()=>{D.current||=requestAnimationFrame(()=>{var e;D.current=0,$(w,String(((e=T.current)==null?void 0:e.scrollTop)||0)),I()})},z=b(()=>g(W,{blocks:y,base:`${J}${l}/`,onAnchor:e=>L(e)}),[y,l]);return m(p,{open:!0,fullWidth:!0,maxWidth:`lg`,onClose:i,slotProps:{paper:{sx:{height:`calc(100% - 64px)`}}},children:[m(h,{sx:{display:`flex`,alignItems:`center`,gap:1,pb:1},children:[g(c,{title:f.t(`Contents`),slotProps:{popper:{sx:{pointerEvents:`none`}}},children:g(u,{size:`small`,color:k?`primary`:`default`,onClick:()=>{$(Y,String(!k)),A(!k)},children:g(S,{})})}),f.t(`Documentation`)]}),m(d,{dividers:!0,sx:{display:`flex`,p:0,minHeight:0},children:[k?m(s,{sx:{flex:`0 0 280px`,minWidth:0,display:`flex`,flexDirection:`column`,minHeight:0,borderRight:1,borderColor:`divider`},children:[g(s,{sx:{p:1},children:g(te,{autoFocus:!0,fullWidth:!0,size:`small`,variant:`outlined`,placeholder:f.t(`Filter`),value:j,onChange:e=>M(e.target.value),onKeyDown:e=>{e.key===`Enter`&&F.length?L(F[0].anchor):e.key===`Escape`&&j&&(e.stopPropagation(),M(``))},slotProps:{input:{endAdornment:j?g(_,{position:`end`,children:g(u,{size:`small`,onClick:()=>M(``),children:g(t,{fontSize:`small`})})}):null}}})}),m(s,{ref:E,sx:{flex:1,overflowY:`auto`,pb:1},children:[F.map(e=>m(s,{"data-anchor":e.anchor,title:e.text,onClick:()=>L(e.anchor),sx:{display:`flex`,alignItems:`baseline`,gap:1,pl:e.level<3?1.5:3,pr:1,py:.4,mt:+(e.level<3&&!j),cursor:`pointer`,fontSize:e.level<3?14:13,fontWeight:e.level<3?500:400,color:e.anchor===N?`primary.main`:`text.primary`,bgcolor:e.anchor===N?`action.selected`:void 0,"&:hover":{bgcolor:`action.hover`}},children:[g(s,{component:`span`,sx:{flexShrink:+!e.code,fontFamily:e.code?X:void 0,overflow:`hidden`,textOverflow:`ellipsis`,whiteSpace:e.code?`nowrap`:void 0,maxWidth:`100%`},children:e.name}),e.description?g(s,{component:`span`,sx:{minWidth:0,fontSize:12,color:`text.secondary`,overflow:`hidden`,textOverflow:`ellipsis`,whiteSpace:`nowrap`},children:e.description}):null]},e.anchor)),F.length?null:g(r,{sx:{px:1.5,py:1,fontSize:13,color:`text.secondary`},children:f.t(`Nothing found`)})]})]}):null,g(s,{ref:T,onScroll:R,sx:{flex:1,minWidth:0,overflowY:`auto`,position:`relative`,px:3,py:2},children:z})]}),g(ee,{children:g(n,{variant:`contained`,color:`grey`,startIcon:g(a,{}),onClick:i,children:f.t(`Close`)})})]})}export{ce as default};