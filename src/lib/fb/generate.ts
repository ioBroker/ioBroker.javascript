import { analyzeGraph, formatIssue, type FbAnalysis } from './analyze';
...
import { jsCode, jsParameters } from './js';
import { parseTime } from './time';
import { factoryName, usedUserBlocks } from './user';
...
const TEXT_PARAMS: string[] = ['OID', 'ENUM', 'NAME', 'INSTANCE', 'CRON', 'CODE'];
...
class BlockWriter {
    ...
    private param(id: string): string {
        ...
        return toLiteral(value, resolvePinType(this.block, type as FbPinType, this.def));
    }
...
interface Code {
    ...
    /** Entries of `S` */
    signals: string[];
    /** The lines of a cycle, one per block */
    body: string[];
    inputs: Set<string>;
    outputs: Set<string>;
}
...
function writeBlocks(graph: FbGraph, analysis: FbAnalysis, userBlocks: Record<string, FbUserBlock>): Code {
    ...
    for (const block of code.blocks) {
        ...
        if (def.user) {
            code.instances.push(`${block.id}: ${factoryName(def.type)}(), /*#fb:${block.id}*/`);
        } else if (def.type === 'JS') {
            ...
            code.instances.push(
                `${block.id}: fb.JS(function (${jsParameters(block, def).join(', ')}) { /*#fb:${block.id}*/\n${jsCode(block, def)}`,
                `}, ${getOutputs(block, def).length}),`,
            );
        } else if (def.stateful) {
            code.instances.push(`${block.id}: fb.${def.type}(), /*#fb:${block.id}*/`);
        }
        getOutputs(block, def).forEach(pin => code.signals.push(`'${block.id}.${pin.id}': ${initialValue(pin.type)},`));
...
function objectLiteral(name: string, entries: string[], depth: number): string[] {
 ...
 */
function writeFactory(user: FbUserBlock, userBlocks: Record<string, FbUserBlock>): string[] {
    const analysis = analyzeGraph(user.graph, { userBlocks, isBlock: true });
    const code = writeBlocks(user.graph, analysis, userBlocks);
    const def = getBlockDef(user.type, userBlocks)!;
    // the pins of a user block always have a signal type of their own
    const pins = [
        ...def.inputs.map(pin => `${pin.id}: ${toLiteral(pin.default, pin.type as FbSignalType)}`),
        ...def.outputs.map(pin => `${pin.id}: ${initialValue(pin.type as FbSignalType)}`),
    ];
    return [
        `// user block ${JSON.stringify(user.name)} (${user.type}), version ${user.version}`,
        `function ${factoryName(user.type)}() {`,
...
export function generateSource(input: FbGraph): FbGenerated {
    ...
    const used = usedUserBlocks(input, input.userBlocks);
    ...
    const top = [
        ...
        `const rt = fb.runtime({ ${SANDBOX_FUNCTIONS.join(', ')} }, '${FB_RUNTIME_VERSION}');`,
        '',
    ...
    const lines = [
        ...top,
        ...indent(
            code.body.map((line, i) => `case ${i}: if (B[${i}] && go !== ${i}) return ${i}; ${line}`),
            2,
        ),
        '    }',
        '    return -1;',
