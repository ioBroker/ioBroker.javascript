...
import { getInputs, getOutputs } from './graph';
import { getBlockDef } from './library';
import type { FbBlock, FbBlockDef, FbGraph, FbLink } from './types';

/** How the editor draws a block, in pixels */
export interface FbBlockMetrics {
    /** From the top of the block to the line under the type */
    header: number;
    /** The line under the type: the state, the value, the pin, the version */
    detail: number;
    /** One row of pins */
    pin: number;
    /** Below the last row */
...
export interface FbLayoutOptions {
    /** The sizes of the blocks as they are drawn, by ID; estimated where missing */
    sizes?: Record<string, { width: number; height: number }>;
    /** Where the pins are - by default as the first editor drew them */
    metrics?: FbBlockMetrics;
}

const DEFAULT_METRICS: FbBlockMetrics = { header: 22, detail: 18, pin: 20, frame: 6 };
...
interface Item {
    id: string;
    block: FbBlock;
    def: FbBlockDef | undefined;
    width: number;
    height: number;
    /** From the top of the block to the first pin */
    pinTop: number;
}

/** Blocks with a line under the type (state, value, pin, version) are that much higher */
function pinTop(def: FbBlockDef | undefined, metrics: FbBlockMetrics): number {
...
function settle(cluster: Cluster): void {
    let offset = 0;
    let sum = 0;
 ...
 */
export function layoutGraph(graph: FbGraph, options: FbLayoutOptions = {}): Record<string, [number, number]> {
    ...
    for (const block of graph.blocks) {
        const def = getBlockDef(block.type, graph.userBlocks);
        const rows = Math.max(getInputs(block, def).length, getOutputs(block, def).length, 1);
    ...
    const incoming = new Map<string, FbLink[]>();
    const outgoing = new Map<string, FbLink[]>();
    ...
    const outputY = (link: FbLink): number => {
        const item = items.get(link.from[0])!;
        const index = getOutputs(item.block, item.def).findIndex(pin => pin.id === link.from[1]);
        return item.pinTop + Math.max(index, 0) * PIN_HEIGHT + PIN_HEIGHT / 2;
    };
    const inputY = (link: FbLink): number => {
        const item = items.get(link.to[0])!;
        const index = getInputs(item.block, item.def).findIndex(pin => pin.id === link.to[1]);
        return item.pinTop + Math.max(index, 0) * PIN_HEIGHT + PIN_HEIGHT / 2;
    };
     ...
     */
    const place = (ids: string[], want: (id: string) => number | null): void => {
        const wishes = ids
    ...
    const fromLeft = (id: string): number | null =>
        mean((incoming.get(id) || []).map(link => y.get(link.from[0])! + outputY(link) - inputY(link)));
    // level with the inputs it feeds
    const fromRight = (id: string): number | null =>
        mean((outgoing.get(id) || []).map(link => y.get(link.to[0])! + inputY(link) - outputY(link)));
