...
const AdditionallyContentBlockItems = ({ itemsSwitchesRender, blockValue, boolean, typeBlock, name, userRules, setUserRules, allBlocks }) => {
    ...
    const [{ canDrop, isOver, offset, targetId }, drop] = useDrop({
        ...
        hover: (item, monitor) => {
            let acceptedBy;
            if (item.object) {
                acceptedBy = item.object.getStaticData().acceptedBy;
            } else {
    ...
    return <div ref={drop} style={{ backgroundColor }} className={`${cls.contentBlockItem} ${boolean ? null : cls.contentHeightOff}`}>
        {itemsSwitchesRender[blockValue]?.filter(el => el.nameBlock === name).map((el, idx) => (
                <DragWrapper typeBlocks={typeBlock} key={el._id} {...el} blockValue={blockValue} allProperties={el} userRules={userRules} setUserRules={setUserRules}>
                    <CurrentItem {...el} blockValue={blockValue} userRules={userRules} setUserRules={setUserRules} allBlocks={allBlocks}/>
                </DragWrapper>))}
...
const ContentBlockItems = ({ typeBlock, name, nameAdditionally, additionally, border, userRules, setUserRules, allBlocks }) => {
